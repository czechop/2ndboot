...
void image_complete() {
   ...
   printf("iniside image_complete\n");

   printf("image_complete: buffers_list[%d]\n", (int)buffers_list);

   for (i = 1; i <= IMG_LAST_TAG; ++i) 
   {
      ab = &buffers_list[i].abstract;
      printf("image_complete: i[%d], ab[%d]\n", i, (int)ab);

      if (ab->state == B_STAT_CREATED) 
      {
         printf("image_complete: ab[%d], state created\n", (int)ab);
      }
      if ((ab->state == B_STAT_COMPLETED) && (ab->attrs & B_ATTR_VERIFY)) 
      {
         ...
         { 
            ...
            printf("image_complete: ab[%d], crc error\n", (int)ab);
         }
         else
         {
            printf("image_complete: ab[%d], crc ok\n", (int)ab);
         }
      }
   }
}
